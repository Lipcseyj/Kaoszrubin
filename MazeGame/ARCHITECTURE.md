...
- A `Game.CreateSessionSnapshot()` a futó host állapotából elkészíti a transport által közvetlenül publikálható session- és world read modelt. Host módban a fő ciklus csak akkor és legfeljebb 10 Hz-cel készít snapshotot, ha van csatlakozott vendég. A world projekció csak a `FogOfWar` szerint ténylegesen felfedett adatokat adja át. A publisher a hostnak minden party-inventoryt átad, a vendég frame-jében viszont csak a hozzá rendelt karakter inventoryja marad. A protokoll 2-es verziójától a `CharacterSheetSnapshot` a tulajdonságokat, XP-t, tehetségneveket, állapotikonokat és megjelenítési metaadatokat is hordozza. A host és a vendég ugyanazt a `CharacterSheetPanel` sorelrendezést használja: a vendégnél is folyamatosan a térkép mellett látszik a karakterlap, az `I` csak inventoryfókuszt kapcsol. Nincs karakterváltás vagy más partitag-lista. A saját explicit slotjai között atomi cserét kérhet, hátizsáktárgyat használhat, tárgyat dobhat le, illetve az aktuális mező kupacából az első tárgyat az első üres hátizsákhelyre veheti fel; minden művelet inventory- és szükség esetén kupacrevízióhoz kötött. Fázisváltáskor az inventoryfókusz bezár, így harc közben az inventory nem módosítható. A szimulációs szálon elutasított commandok címzetten visszajutnak hozzá. Távoli vereségnél a karakter tetemmé válik, a session feloldja a vezérlését, a vendég pedig megfigyelő módban marad. A session vezérlési állapota és a party-lista többszálú olvasás ellen védett. A vendég jelenlegi harci felülete csak fegyveres támadást és halottűzést kínál; a távoli varázslatcélzás és a szintlépési választások tulajdonoshoz irányítása a következő UI/protokoll-lépés.
