...
- `Transport/SignalR`: beágyazható Kestrel/SignalR LAN host. A `/coop` Hub egyetlen `SendWire` metódussal fogad, a `ReceiveWire` kliensmetódussal küld, és kizárólag a `CoopHostGateway` eredményét továbbítja. A `CoopSignalRServer.PublishSnapshotAsync` a host játékhurkába köthető publikálási pont; az alapértelmezett `http://0.0.0.0:5127` csak LAN-fejlesztési végpont, publikus internetre TLS/relay nélkül nem tehető ki.
...
- A `Game.CreateSessionSnapshot()` a futó host állapotából elkészíti a transport által közvetlenül publikálható session- és world read modelt. A world projekció csak a `FogOfWar` szerint ténylegesen felfedett adatokat adja át. A publisher a hostnak minden party-inventoryt átad, a vendég frame-jében viszont csak a hozzá rendelt karakter inventoryja marad. A helyi inventory UI mozgatáskor, használatkor és eldobáskor sem módosít közvetlenül állapotot: szemantikus commandot küld, amelyet a host dolgoz fel. A földi loot explicit pickup-commandja készen áll a távoli klienshez; a leader jelenlegi keresés-parancsa továbbra is automatikusan osztja szét a felvehető lootot. A transportfüggetlen handshake, wire codec, gateway, in-memory kapcsolat, SignalR/LAN host és kliensoldali snapshot/delta read model működik; a következő réteg a konkrét SignalR klienskapcsolat és a host/join UI bekötése.
