...
- A `Game.CreateSessionSnapshot()` a futó host állapotából elkészíti a transport által közvetlenül publikálható session- és world read modelt. A world projekció csak a `FogOfWar` szerint látható adatokat adja át, így a kliens nem kapja meg előre a labirintust vagy a rejtett ellenfeleket. Két ilyen snapshotból a `WorldDeltaProjector` előállítja a sorrendhelyes world deltát. A teljes karakter-inventory, a session-karakter deltái és a transport-publisher még külön szerződésként következnek, hogy ne a mentési formátum váljon hálózati API-vá.
