...
> Implementációs állapot (2026-08-26): az első session-alap elkészült. Van stabil, mentett `CharacterId`, `PlayerId`, karaktervezérlési tulajdon, fázis- és sorszám-validált command queue, sorrendezett session-eseményfolyam, remote NPC-átvétel, disconnect → AI és reconnect. A térképi, harci és inventory inputok szemantikus, host-validált commandként működnek. Elkészült a protokollverziózott `SessionSnapshot`, a felfedett world read model, a sorszámkötött `WorldDelta`, valamint a kliensenkénti teljes snapshot → ACK → delta/resync publisher. A transportfüggetlen protokollhatár verzió- és SHA-256 katalógushash-handshake-et, 256 bites reconnect-tokent, explicit allowlistes JSON codecet, karakterátvételt és snapshot ACK/resync üzeneteket ad. A `CoopHostGateway` a transportkapcsolathoz köti a `PlayerId`-t, elutasítja a sender-hamisítást, és kezeli a teljes kapcsolat-életciklust. A Kestrel/SignalR LAN host `/coop` végpontja csak wire üzeneteket továbbít, a snapshotok címzett publikálását külön API-n fogadja. A kliensoldali `ClientSessionStore` teljes snapshotot és deltát alkalmaz, régi ACK-baseline-okat is helyesen kezel, majd ACK-ot vagy resync-kérést generál. A `CoopSignalRClient` kezeli a hello/reconnect állapotgépet, karakterátvételt, automatikus ACK/resyncet és commandküldést; valódi loopback Kestrel teszt fedi le a hello → NPC-átvétel → snapshot → command folyamatot. A csata akcióra várása nem használ blokkoló belső `ReadKey`-t; a helyi célzó overlay még modális UI. A host/join menü, a távoli renderer/input és a fogadói read model még hátravan.

Az eredeti renderer nélküli `GameSession`, a protokollhatár, a host-oldali SignalR/LAN adapter, a kliensoldali read-model/reducer és a SignalR klienskapcsolat elkészült. A következő konkrét lépés egy egyszerű host/join indítási workflow: a host játéka indítsa és periodikusan táplálja a szervert, a vendég kérje be a címet és karaktert, majd a hálózati read modelből rajzoljon és oda küldje az inputot.
