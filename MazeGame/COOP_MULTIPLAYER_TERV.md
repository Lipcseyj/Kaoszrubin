...
> Implementációs állapot (2026-08-26): az első session-alap elkészült. Van stabil, mentett `CharacterId`, `PlayerId`, karaktervezérlési tulajdon, fázis- és sorszám-validált command queue, sorrendezett session-eseményfolyam, remote NPC-átvétel, disconnect → AI és reconnect. A térképi és harci inputok szemantikus, host-validált commandként működnek. Elkészült a protokollverziózott `SessionSnapshot`, a felfedett world read model, a sorszámkötött `WorldDelta`, valamint a kliensenkénti teljes snapshot → ACK → delta/resync publisher. Az explicit slotos inventory read model revíziózott és címzett szerint szűrt. A mozgatás/csere, fogyóeszköz-használat, eldobás és pickup mind slot- és revízióalapú commandot használ; a kliens tárgyat, töltetet vagy hatáseredményt nem diktálhat. A `GroundItemPile` külön revíziót és itemenkénti töltetet őriz, amely snapshotban és mentésben is megmarad. A csata akcióra várása nem használ blokkoló belső `ReadKey`-t; a helyi célzó overlay még modális UI. A konkrét SignalR transport és fogadói read model még hátravan.
...
1. két karaktert külön mozgatni;
2. a hoston validálni a foglaltságot;
3. típusos eseményként visszaadni az eredményt;
4. disconnectkor az egyik karaktert NPC-vezérlésre visszaállítani.

Ha ez a határfelület jól sikerül, ugyanarra később rádugható a konzol, a LAN SignalR és az internetes relay anélkül, hogy a játékszabályokat három helyen kellene újraírni.
