...
> Implementációs állapot (2026-08-26): az első session-alap elkészült. Van stabil, mentett `CharacterId`, `PlayerId`, karaktervezérlési tulajdon, fázis- és sorszám-validált command queue, sorrendezett session-eseményfolyam, remote NPC-átvétel, disconnect → AI és reconnect. A helyi leader térképi parancsai már ezen az útvonalon mennek. A harc `BattleId`/`TurnId`-val azonosított, egy akciót végrehajtó `BattleState` állapotgépet használ. A fizikai támadás, a varázslat és a halottűzés is szemantikus, host-validált commandként működik. Elkészült a protokollverziózott `SessionSnapshot`, a felfedett world read model, a sorszámkötött `WorldDelta`, valamint a kliensenkénti teljes snapshot → ACK → delta/resync publisher. Az explicit slotos inventory read model revíziózott és címzett szerint szűrt. Az `InventoryTransferCommand` kizárólag slotcímeket és revíziókat hordoz; a host atomi, töltetmegőrző cserét végez. A vendég csak saját inventoryján belül rendezhet, a host party-karakterek között is. A csata akcióra várása nem használ blokkoló belső `ReadKey`-t; a helyi célzó overlay még modális UI. A konkrét SignalR transport és fogadói read model még hátravan.
