...
public sealed class LiveCharacter
{
    ...
    {
        var upgrade = global::KaoszRubin.Domain.Characters.ClassFeatureUpgrades.Find(id);
    ...
    public EquipmentWearResult ApplyInventoryItemWear(InventorySlotKind kind, int index, EquipmentWearCause cause, int amount)
    {
        if (amount < 0)
        ...
        if (item is null || state is null || amount == 0)
            return EquipmentWearResult.None;

        var maximum = EquipmentDurabilityRules.MaximumDurability(item);
        if (maximum <= 0)
            return EquipmentWearResult.None;

        amount = EquipmentDurabilityRules.WearAfterClassBenefit(this, kind, item, cause, amount);

        if (amount == 0)
            return EquipmentWearResult.None;

        var previousDurability =
            EquipmentDurabilityRules.CurrentDurability(item, state.Value);

        var previousCondition =
            EquipmentDurabilityRules.Condition(maximum, state.Value.DurabilityDamage);

        var wornState =
            EquipmentDurabilityRules.ApplyWear(item, state.Value, amount);

        var currentDurability =
            EquipmentDurabilityRules.CurrentDurability(item, wornState);

        var currentCondition =
            EquipmentDurabilityRules.Condition(maximum, wornState.DurabilityDamage);

        if (currentDurability == previousDurability)
        {
            return new EquipmentWearResult(false, maximum, previousDurability, currentDurability, previousCondition, currentCondition);
        }
        ...
        ApplyInventoryChanges(
            new InventorySlotChange(
                ...
                index,
                item,
                GetInventoryItemCharges(kind, index),
                GetInventoryItemQuantity(kind, index),
                wornState));

        return new EquipmentWearResult(true, maximum, previousDurability, currentDurability, previousCondition, currentCondition);
    }
