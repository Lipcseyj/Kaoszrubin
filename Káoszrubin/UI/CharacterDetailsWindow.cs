...
public static class CharacterDetailsWindow
{
        ...
        GameDataCatalog data)
    {
        ...
        var lines = new List<(string, ConsoleColor)>
        {
            ...
            ($"⚔ ⚖ Harci terhelés: {sheet.EquippedWeight}/{sheet.CombatCarryingCapacity:0.##} — {sheet.Encumbrance}", LoadColor(sheet.Encumbrance)),
    ...
    private static void Draw(IReadOnlyList<(string Text, ConsoleColor Color)> lines)
    {
        ...
        for (var index = 0; index < lines.Count; index++)
        {
            ...
            Write(left + sides.Left.Length, top + index + 1,
                " " + PadRightDisplay(text, contentWidth) + " ", lines[index].Color);
