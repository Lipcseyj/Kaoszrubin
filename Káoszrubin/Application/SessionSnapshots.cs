...
public sealed record LevelUpPromptSnapshot(Guid PromptId, CharacterId CharacterId, string CharacterName,
    ...
    IReadOnlyList<LevelUpBonusSnapshot>? Bonuses = null,
    IReadOnlyList<LevelUpTextLineSnapshot>? ContextLines = null);

public sealed record LevelUpBonusSnapshot(int Level, int Vitality, int Mana);
public sealed record LevelUpTextLineSnapshot(string Text, ConsoleColor Color);
...
public sealed record CharacterRestSnapshot(CharacterId CharacterId, string CharacterName, ConsoleColor Color,
    int HealedAmount, int ManaRestored, int CurrentVitality, int MaximumVitality, int CurrentMana, int MaximumMana,
    bool UsesMana, IReadOnlyList<string> RemovedNegativeStatuses);
...
public sealed record TacticalBattleParticipantSnapshot(CombatantId Id, string Name, BattleSide Side,
    ...
    bool IsCurrent, bool IsCurrentTarget = false, WorldEntityId? EnemyId = null,
    IReadOnlyList<CombatConditionSnapshot>? Conditions = null);
